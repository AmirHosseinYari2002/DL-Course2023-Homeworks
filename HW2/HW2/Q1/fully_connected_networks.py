...
class Linear(object):
    ...
    def forward(x, w, b):
        ...
        ######################################################################
        # Reshape the input into a matrix (N, D), where D = d_1 * ... * d_k
    ...
    def backward(dout, cache):
        ...
        dx = dout.mm(w.t())
        # Reshape the gradient back to the original shape of x
        dx = dx.view(x.size())
...
class Linear_ReLU(object):
    ...
    def forward(x, w, b):
        ...
        # Combine caches for backward pass
    ...
    def backward(dout, cache):
        ...
        # ReLU backward pass
        dfc_out = ReLU.backward(dout, relu_cache)

        # Linear backward pass
        dx, dw, db = Linear.backward(dfc_out, fc_cache)
        ######################################################################
        #                END OF YOUR CODE                                    #
        ######################################################################
        return dx, dw, db
...
def softmax_loss(x, y):
    ...
    shifted_x = x - x.max(dim=1, keepdim=True).values

    # Compute softmax scores
    exp_scores = torch.exp(shifted_x)
    softmax_scores = exp_scores / exp_scores.sum(dim=1, keepdim=True)

    # Compute the negative log likelihood loss
    N = x.shape[0]
    loss = -torch.log(softmax_scores[range(N), y]).sum() / N

    # Compute the gradient of the softmax loss with respect to x
    dx = softmax_scores.clone()
    dx[range(N), y] -= 1
    dx /= N
    ######################################################################
    #                END OF YOUR CODE                                    #
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        # Backward pass
        dh1, grads['W2'], grads['b2'] = Linear.backward(dscores, cache_scores)
